...
(function( $ ) {
  $.fn.freezeTableColumns = function(options) {
    options = initOptions(options);
      ...
      '<div id="' + main_id + '-region2" style="display: inline-block; vertical-align: top; width: 5px; overflow-x: hidden; overflow-y: scroll;"><div></div></div>' + //scroll judul row
		  '</div>'
		);
		main_div.append(
  ...
  */
  $.fn.freezeTableColumnsLayout = function(options) {
    ...
    $('#' + main_id + '-region1').children('div').width(1200);
    $('#' + main_id + '-region2').children('div').width(1200);
    $('#' + main_id + '-region3').children('div').width(1200);
    $('#' + main_id + '-region4').children('div').width(1200);
